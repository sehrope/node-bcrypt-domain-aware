...
test: compile
	node_modules/.bin/mocha --reporter tap $(TESTARGS)
...
test-cov: compile-cov
	...
	mkdir -p cov
	COVERAGE=true node_modules/.bin/mocha --reporter html-cov --require ./lib-cov/init.js $(TESTARGS) > cov/coffee.html
...
test-cov-js: compile-cov-js
	rm -rf cov/js.html
	mkdir -p cov
	COVERAGE=true node_modules/.bin/mocha --reporter html-cov  $(TESTARGS) > cov/js.html
	open cov/js.html
